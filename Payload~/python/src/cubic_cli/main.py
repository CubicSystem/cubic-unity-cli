...
def build_client(args: argparse.Namespace) -> UnityClient:
    ...
    while True:
        ...
        if time.monotonic() >= deadline:
            raise last_error if last_error is not None else DiscoveryError("No active Cubic Unity instance was found.")
...
def build_command_activity_timeout_state(
    ...
    state: Dict[str, Any] = {
        ...
        "message": (
            f"Cubic Unity CLI is still busy with {command!r} after "
...
def is_transient_command_error(exc: ClientError) -> bool:
    ...
    return any(
        marker in message
        for marker in (
            "another cubic command is already running",
            "command_busy",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="cubic-cli", description="Cubic Unity CLI")
    parser.add_argument("--version", action="version", version=f"cubic-cli {__version__}")
    ...
    component_get.add_argument("--read-all-members", dest="readAllMembers", action="store_true")
    ...
    runtime_inspect.add_argument("--member")
    runtime_inspect.add_argument("--read-all-members", dest="readAllMembers", action="store_true")
    runtime_inspect.add_argument("--include-components", dest="includeComponents", action="store_true")
    runtime_inspect.add_argument("--max-depth", dest="maxDepth", type=int, default=1)
