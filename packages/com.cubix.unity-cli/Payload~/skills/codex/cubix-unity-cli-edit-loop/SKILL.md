...
3. Run `cubix-cli verify` only after creating, editing, or deleting `Assets/**/*.cs`.
4. Continue with `scene`, `object`, `prefab`, or `runtime` commands only when verify succeeds.
5. Prefer structured commands over `cubix-cli exec csharp`.
