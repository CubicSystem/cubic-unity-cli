...
{
    internal interface ICubixCliCommandHandler
    {
    ...
    internal static class CommandRouter
    {
        ...
        {
            public CubixCliCommandAttribute Attribute { get; set; }
            public ICubixCliCommandHandler Handler { get; set; }
        }
        ...
        static CommandRouter()
        {
            ...
            var handlerTypes = AppDomain.CurrentDomain.GetAssemblies()
                ...
                .Where(type => !type.IsAbstract && typeof(ICubixCliCommandHandler).IsAssignableFrom(type));

            foreach (var handlerType in handlerTypes)
            {
                var attribute = (CubixCliCommandAttribute)Attribute.GetCustomAttribute(handlerType, typeof(CubixCliCommandAttribute));
                if (attribute == null || string.IsNullOrWhiteSpace(attribute.Group))
                {
                    continue;
                }

                if (!(Activator.CreateInstance(handlerType) is ICubixCliCommandHandler handler))
                {
                    continue;
                }
        ...
        public static CommandPreflightResult Preflight(CommandRequest request)
        {
            ...
            if (!(registration.Handler is ICubixCliPreflightHandler preflightHandler))
            {
