...
`com.cubicengine.unity-cli` is a Unity editor package that hosts the Cubic Unity CLI connector
...
- Discovery and heartbeat files under `%USERPROFILE%\.cubic-cli`
...
- Setup window at `Tools/CubicEngine/UnityCli`
- Packaged Python CLI payload installed as `cubic-cli`
- Packaged Codex and Claude Code skills

## Setup Window

Open `Tools/CubicEngine/UnityCli` to manage:
...
- `CLI`
  - Detect Python, pip, pipx, and installed `cubic-cli`
  - `Install CLI`
  - `Update CLI`
  - `Repair CLI`
  - `Uninstall CLI`
...
The packaged `cubic-cli` command includes:
