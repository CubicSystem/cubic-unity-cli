...
namespace CubicEngine.UnityCli
{
    public sealed class ExecContext
    {
        ...
        {
            _logs.Add(message);
            Debug.Log("[cubic-exec] " + message);
        }
    }

    [CubicCliCommand(Group = "exec", Name = "exec", Description = "Run one-off editor C# snippets.")]
    internal sealed class ExecCommand : ICubicCliCommandHandler, ICubicCliPreflightHandler
    {
        ...
        public object Execute(string action, JObject parameters)
        {
            ...
            try
            {
                ...
                var type = assembly.GetType("CubicCliDynamic.Executor");
        ...
        private static string BuildSource(string code, JArray usings)
        {
            ...
            return string.Join(Environment.NewLine, usingLines.Distinct()) + Environment.NewLine +
                "namespace CubicCliDynamic" + Environment.NewLine +
                "{" + Environment.NewLine +
