...
namespace CubicEngine.UnityCli
{
    internal sealed class UnityCliSetupWindow : EditorWindow
    {
        ...
        [MenuItem("Tools/CubicEngine/UnityCli")]
        ...
        private void DrawHeader()
        {
            ...
            {
                EditorGUILayout.LabelField("Cubic Unity CLI", _sectionTitleStyle);
                DrawSectionDivider();
                ...
                if (PackageLayout.HasLoadedPackageDrift)
                {
                    EditorGUILayout.HelpBox(
                        "The loaded Cubic Unity CLI package version does not match the resolved project package version. Use reload only as a fallback; if it still stays stale afterward, restart Unity.",
                        MessageType.Warning);
                }
        ...
        private void DrawCliSection()
        {
            DrawSection("CLI", () =>
            {
                ...
                DrawColoredRow("cubic-cli Package", BuildCliPackageLabel(), _cliStatus.cliInstalled && _cliStatus.cliVersionMatches);
                DrawColoredRow("cubic-cli Command", BuildCliCommandLabel(), _cliStatus.cliCommandAvailable && _cliStatus.cliCommandVersionMatches);
                ...
                {
                    EditorGUILayout.HelpBox("Installed cubic-cli version does not match the expected CLI version. Reinstall CLI.", MessageType.Warning);
                }
        ...
        {
            PackageReloadService.RequestReload("Queued a Cubic Unity CLI package reload from the setup window.");
        ...
        private string BuildCliDiagnosticsText()
        {
            ...
            builder.AppendLine("- dynamic invocation: cubic-cli call");
            builder.AppendLine("- safety checks: cubic-cli preflight");
            builder.AppendLine("- batch execution: cubic-cli batch");
            builder.AppendLine("- operational commands: status, menu, refresh, reserialize");
            builder.AppendLine("- top-level tests: cubic-cli test run / cubic-cli test status");
