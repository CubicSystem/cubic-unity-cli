...
namespace CubicEngine.UnityCli
{
    ...
    {
        private const string CliPackageName = "cubic-unity-cli";
        ...
        public static CliInstallationStatus Inspect()
        {
            ...
            diagnostics.AppendLine("cubic-cli package: " + (status.cliInstalled ? status.cliVersion ?? "installed" : "missing"));
            diagnostics.AppendLine("Package matches expected CLI: " + (status.cliVersionMatches ? "yes" : "no"));
            diagnostics.AppendLine("cubic-cli command: " + (status.cliCommandAvailable ? status.cliCommandVersion ?? "available" : "missing"));
        ...
        private static string InstallOrRepairCli(bool reinstall)
        {
            ...
            log.AppendLine("Cubic Unity CLI installation");
            log.AppendLine();
        ...
        private static bool TryResolveCliCommand(string preferredPath, out CommandSpec cliCommand, out string version, out string message)
        {
            ...
            AddCommandCandidate(candidates, Path.Combine(userProfile, ".local", "bin", "cubic-cli.exe"));
            AddCommandCandidate(candidates, Path.Combine(userProfile, "pipx", "venvs", CliPackageName, "Scripts", "cubic-cli.exe"));
