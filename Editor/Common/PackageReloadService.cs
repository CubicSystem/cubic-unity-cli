    ...
    internal static class PackageReloadService
    {
        ...
        public static void RequestReload(string reason)
        {
            ...
            SetStatus(string.IsNullOrWhiteSpace(reason)
                ? "Queued a Cubic Unity CLI package reload."
                : reason);
        }
        ...
        private static void Pump()
        {
            ...
            if (ResolveWasRequested)
            {
                if (EditorApplication.isUpdating || EditorApplication.isCompiling || ConnectionService.IsReloading)
                {
                    ...
                    SetStatus("Waiting for Unity Package Manager and script reload to finish updating Cubic Unity CLI.");
                    return;
                }
                ...
                ClearPending("Triggered a Cubic Unity CLI package reload. If the loaded package stays stale after compilation, restart Unity.");
                return;
            }
            ...
            try
            {
                ...
                _nextAttemptAt = EditorApplication.timeSinceStartup + RetryIntervalSeconds;
                SetStatus("Triggered a Cubic Unity CLI package resolve. Waiting for Package Manager processing.");
            }
            catch (System.Exception exception)
            {
                ClearPending("Could not trigger a Cubic Unity CLI package reload: " + exception.Message);
            }
        }
